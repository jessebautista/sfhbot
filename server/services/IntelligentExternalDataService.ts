...
export class IntelligentExternalDataService {
  ...
  private async initializeConnection() {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.log('⚠️ External database credentials not found - service unavailable')
    ...
    try {
      ...
      if (error) {
        ...
        console.log('✅ Intelligent external database service initialized')
  ...
  async intelligentSearch(userQuery: string): Promise<SearchResult> {
    ...
    executionLog.push(`🧠 Starting intelligent search for: "${userQuery}"`)

    if (!this.isAvailable || !this.externalSupabase) {
      executionLog.push('❌ External database not available - using fallback')
      return {
        results: [],
        query: userQuery,
    ...
    try {
      ...
      executionLog.push(`🎯 Intent: ${intent.type} (${Math.round(intent.confidence * 100)}% confidence)`)
      executionLog.push(`📋 Generated ${sqlQueries.length} targeted queries`)

      // Step 2: Execute the AI-generated queries
      const allResults: any[] = []
      
      for (const sqlQuery of sqlQueries) {
        executionLog.push(`🗄️ Executing: ${sqlQuery.table} query - ${sqlQuery.reasoning}`)
        
        try {
          const results = await this.executeIntelligentQuery(sqlQuery.table, sqlQuery, userQuery)
          if (results.length > 0) {
            allResults.push(...results)
            executionLog.push(`✅ Found ${results.length} results from ${sqlQuery.table}`)
          } else {
            executionLog.push(`📭 No results from ${sqlQuery.table}`)
          }
        } catch (error) {
          executionLog.push(`❌ Error querying ${sqlQuery.table}: ${error}`)
      ...
      executionLog.push(`📝 Processed ${allResults.length} results into natural language`)
      ...
      executionLog.push(`❌ Error in intelligent search: ${error}`)
  ...
  private async fallbackSearch(userQuery: string, executionLog: string[]): Promise<SearchResult> {
    executionLog.push('🔄 Using fallback basic search due to AI reasoning failure')
    ...
    try {
      ...
      executionLog.push(`🔄 Fallback found ${allResults.length} total results`)
      ...
      executionLog.push(`❌ Fallback search also failed: ${error}`)
      return {
        results: [],
        query: userQuery,
